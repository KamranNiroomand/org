 ...
 *
 * Held as static data rather than fetched, for two reasons. The membership
 * changes a handful of times a year, so a live dependency buys accuracy that
 * nobody would notice while adding a network call that can fail. And the
 * sector arrives with it — which means the market map needs no per-symbol
 * profile lookup at all, only the batched quote call.
 *
 * Class shares are written with a hyphen (BRK-B), which is what Yahoo expects;
 * the published lists use a dot.
 *
 * Sourced from Wikipedia's list of S&P 500 companies, 2026-08-17.
 */
export const SP500: Array<[symbol: string, name: string, sector: string]> = [
  ['MMM', '3M', 'Industrials'],
...
export const SP500_SECTORS = [...new Set(SP500.map((r) => r[2]))].sort();
